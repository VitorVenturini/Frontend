...
import { useMicrosoftCalendar } from "@/components/microsoftCalendars/microsoftCalendarContext";
...
export default function ModalMicrosoftCalendar({
  selectedUser,
  selectedPage,
  clickedPosition,
  existingButton,
  ...
  const { microsoftCalendars } = useMicrosoftCalendar();
  ...
  const handleCalendarButton = (value: string) => {
    const name = microsoftCalendars.find((c)=>{return c.id == value}).name
  ...
  const handleMicrosoftCalendarsList = () => {
    if (!isCalendarRequested) {
      console.warn("Passou aqui !!!!!!!!");
      wss?.sendMessage({
        api: "admin",
        mt: "RequestMicrosoftCalendars"
      });
      setIsCalendarRequested(true); // Atualiza o estado para evitar reenvio
    }
  }
  // Chamada automática do `handleMicrosoftApiStatus` ao carregar o componente
  useEffect(() => {
    handleMicrosoftCalendarsList();
  }, []);
  return (
    <>
      {isUpdate && (
          ...
          <Select value={calendarButton} onValueChange={handleCalendarButton}>
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  {microsoftCalendars?.map((c, index) => (
                    <SelectItem key={index} value={c.id as string}>
                      {c.name}
                    </SelectItem>
                  ))}
                </SelectGroup>
